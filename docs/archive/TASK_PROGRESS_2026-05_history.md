...
- 15/15 testiä `test_feature_drift.py` passing, 2.71 s
- **K1-detektio nimenomaisesti testattu** — vakaa baseline + +10 hyppy → CRITICAL. Tämä on **olennainen** turvaverkko: jos vastaavaa K1-tyyppistä leakagea tulee tulevaisuudessa, drift-monitorointi nappaa sen viikossa eikä kuukausissa
- Hetzner-cron sunnuntaisin 02:00 → ensimmäinen vertailu sunnuntaina 2026-05-17 (ensi viikko)
- Logitus `data/logs/drift_cron.log`:iin → seurattavissa myöhemmin

### Erityishuomio: kynnysarvojen alkuvaiheen viritys

[feature_drift.py](src/monitoring/feature_drift.py):n alle 3 vk:n historialla käytetään **raw 20 %** rajaa, sen jälkeen 2σ. Tämä on **järkevää alkuvaiheen mitoitusta** — pieni historia ei salli luotettavaa σ-laskentaa, joten raw raja toimii varahälytyksenä. Hyvä insinööritapa.

Kun dataa kertyy 8+ viikkoa, harkitse kynnysarvojen tiukentamista (esim. 1.5σ alemmaksi) — mutta odota ensin 4–6 viikkoa nähdäksesi luonnollinen viikkovaihtelu. Kirjaa TODO tiedostoon.

### Tämä on iso askel projektille

Vaiheen A K1-bugi olisi havaittu **välittömästi** jos tämä monitorointi olisi ollut paikallaan. Nyt sama suoja on tulevaisuuden bugeille. **C1 oli tärkein C-vaiheen tehtävä** ja se on tehty huolellisesti.
...
ei saa. Priorisoitava Vaihe 2:ssa.

**Auditoijan tarkistus tästä erityislöydöksestä:** 🎯 **TÄMÄ ON ISO LÖYDÖS — STRATEGINEN MUUTOS** 14.5.2026 (Opus 4.7)

### Miksi tämä on niin tärkeää

Tulkinta (c) — **asiantuntijan per-hevonen, per-lähtö pace-arvio** — tarkoittaa että `start_interval_group` on:

1. **Asiantuntija-arvio**, joka on alalla tunnettu osana parhaita malleja
2. **Pre-race** (täytetty myös struken-hevosille → ei post-race-vuotoriskiä)
3. **Per-lähtö-spesifi** — ei vain hevosen kiinteä luokitus vaan **tilanteellinen ennuste** miten hevonen pelannee TÄSSÄ kilpailussa
4. **Lähinnä pace-piirrettä** mitä saadaan ilman manuaalista raviraportti-scrapingia
5. **Ainoa lähde** — ei saatavilla ATG:sta eikä Travsportista

**Esimerkki konkretisoi:** hevonen 764491 oli group=1 (nopein aloittaja) kierroksella 171600, mutta group=31 (hidas aloittaja) kierroksella 171100. Sama hevonen, eri ennusteet. Tämä **EI ole hevosen kiinteä ominaisuus** — se on asiantuntijan päätös perustuen lähtöradan, vastustajien, distan ja muiden tekijöiden yhdistelmään.

### Mitä tämä tarkoittaa C3 pace-pilotille

Aiempi oletus: C3 vaatii **manuaalista scrapingia** Travsport-raviraporteista (10+ tuntia työtä, kestää viikkoja).

Nyt: C3-pace-piirre on **suoraan saatavilla `start_interval_group`-kenttänä** Travronden-rajapinnasta. **Tämä on alkuperäisen TASK_TRAVRONDEN_INVESTIGATION.md:n "Vaihe 1 oikotie pace-pilottiin" -hypoteesin toteutuminen** — vaikka eri kenttä (start_interval_group eikä speed).

**C3 voi olla ohitettavissa kokonaan**, mutta varauksellisesti — start_interval_group:n kattavuus on noin 60 %, joten 40 % lähdöistä saa NaN. Tämä on osittainen pace-piirre, ei täydellinen.

### Strateginen suositus — päivitetty aikataulu

**Travronden Vaihe 2 nostetaan prioriteettiin** (oli "kuukauden sisällä", nyt voi olla "viikon sisällä"):

```
VIIKON SISÄLLÄ (uusi prioriteetti):
  1. Travronden Vaihe 2 — kerää 100 vanhaa kierrosta + tallenna piirteet
     - is_first_after_castration, is_first_new_driver/trainer/shoes/carriage
     - start_interval_group  ← pace-piirre
     - speed_records.K/M/L   ← rikkaampi atg_best_km
     - expected_odds         ← markkina-arvio toiselta lähteeltä
     - game_percent.ATG.V*   ← markkinasentimentti
  2. C2 — walk-forward-dokumentaatio (~30 min, halpa)

KUUKAUDEN SISÄLLÄ:
  • Mallin uudelleentreenaus rikastetulla featuristolla
  • A/B-vertailu: ilman Travronden vs. kanssa → mittaa Brier-paranema
  • Päätös: kannattaako Travronden-keräys lisätä scheduler-jobiin tuotantoon

KUN 8+ VIIKKOA DATAA (~2026-07-07):
  • Sire-piirteiden palautus (KNOWN_ISSUES #13)
  • Travronden-tuotantointegraatio
  • Stop/go-päätös (C2-vaatimus täyttyy)

EI ENÄÄ TARVITSE:
  • C3 manuaalinen pace-pilotti — start_interval_group korvaa
  • (Mahdollisesti tehdään vain jos start_interval_group ei riitä tuotannossa)
```

### Realistiset odotukset Brier-vaikutuksesta

**Älä yliarvioi vaikutusta.** Vaikka start_interval_group on pace-piirre, sen lisäys mallille:
- Voi parantaa Brier-arvoa **0.005–0.020** (alalla tyypillinen pace-piirre-vaikutus)
- Voi tuoda valuepelejä jotka ovat **markkinan väärin hinnoittelemia**
- **Mutta** kattavuus 60 % → toimii vain V-pelilähdöissä, ei kaikissa
- LightGBM hoitaa NaN — ei rikkoudu, mutta ei myöskään ennusta paremmin niiden lähtöjen kohdalla

Voittosignaali on tällä hetkellä 0.0025 (Brier vs. uniform). Jos start_interval_group nostaa sen vaikkapa 0.010:een, **se on 4× parannus** — merkittävä mutta ei vielä riittävä tuotantopelaamiseen.

### Mitä on opittavissa tästä päivästä

Yksi pieni 30 min:n tutkimus paljasti **strategisen tason löydöksen**. Tämä on hyvä muistutus että:

1. **Empiirinen tutkimus voittaa spekuloinnin** — alkuperäinen oletus oli että `start_interval_group` voisi olla vain ryhmittely, mutta vaihtelun tarkistaminen osoitti sen olevan asiantuntija-arvio
2. **Halvat selvitykset kannattaa tehdä** — 30 min:n työ siirsi C3:n prioriteetin täysin
3. **Älä luota dokumentaatioon, vaan dataan** — Travrondenin docs ei kerro tätä, vaan empiirinen tarkistus paljasti

### Kokonaisarvio päivästä

Tämä on **tutkimuspäivän kuningasta**. Päivän aikana:
1. ✅ Sire-LOO-korjaus + ablation → vahva tieteellinen tulos
2. ✅ FEATURE_COLS-puhdistus + KNOWN_ISSUES #13
3. ✅ Travronden Vaihe 1 → speed-leakage-vältit + start_interval_group -löydös
4. ✅ C1 drift-monitorointi → tärkein infrastruktuuri-tehtävä valmis
5. ✅ start_interval_group -selvitys → strateginen muutos C3:lle

**Lopeta tänään ehdottomasti tähän.** Päivä on ollut huipputuottelias, ja seuraavan vaiheen (Travronden Vaihe 2) suunnittelu on parempi tehdä levänneenä huomenna tai viikon aikana.

Hyvää työtä — tämä on **mallin kehittämistä parhaimmillaan**.
