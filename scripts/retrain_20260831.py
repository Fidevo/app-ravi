"""Retrain 31.8.2026 — kalibroinnin päivitys tuoreella datalla.

Konteksti (auditointi 31.8.2026): edellinen malli (model_baseline_20260704)
treenattu datalla < 15.6.2026, kalibroitu ikkunalla 15.6.-5.7.2026. Sen
jälkeen kertynyt 1377+ uutta lähtöä. Kalibrointi (T + blend-alpha) on ollut
~2 kk vanha. Tämä ajo PÄIVITTÄÄ kalibroinnin tuoreella OOS-ikkunalla; se ei
lisää uusia piirteitä (FEATURE_COLS koskematon, gatetut piirteet edelleen
pois päältä).

HUOM: ei muuta kuin retrain_20260704.py paitsi CAL_START/TEST_START/OUT_PATH
siirretty eteenpäin datan mukana. Sama 3-jakoinen split (train/kalibrointi/
testi) OOS-harhan välttämiseksi, sama float32-downcast OOM:n torjuntaan.

Aja palvelimella: /home/ravi/app-ravi/.venv/bin/python scripts/retrain_20260831.py
"""
import sys
...
OUT_PATH = f"{DATA_DIR}/model_baseline_20260831.lgb"
CAL_START = "2026-08-01"   # train < tämä
TEST_START = "2026-08-15"  # kalibrointi [CAL_START, TEST_START), testi >= tämä
...
# Kertoimet talteen blend-alfan sovitusta varten.
...
print(f"Blend-alpha (cal-ikkuna): {blend_alpha:.4f}")

# -- Evaluointi testi-ikkunassa (täysin OOS) ---------------------------------
...
# -- Vertailu: markkinan oma Brier samalta testi-ikkunalta (sama baseline --
#    kuin kesäkuun päättävässä testissä) --------------------------------------
tm_odds = tm.merge(odds_lookup, on=["race_id", "horse_id"], how="left")
tm_odds = tm_odds.rename(columns={"win_odds_final": "win_odds"})
tm_odds = tm_odds[tm_odds["win_odds"].notna() & (tm_odds["win_odds"] > 1.0)]
if len(tm_odds) > 0:
    tm_odds["market_prob_raw"] = 1.0 / tm_odds["win_odds"]
    race_vig = tm_odds.groupby("race_id")["market_prob_raw"].transform("sum")
    tm_odds["market_prob"] = tm_odds["market_prob_raw"] / race_vig
    brier_market = float(((tm_odds["market_prob"] - tm_odds["actual_win"]) ** 2).mean())
    print(f"Testi-Brier (markkina, devigattu, n={len(tm_odds)}): {brier_market:.4f}")
    print(f"delta Brier (malli - markkina): {brier_test - brier_market:+.4f}  (negatiivinen = malli parempi)")
else:
    brier_market = None
    print("Markkinavertailu ohitettu: ei kertoimia testi-ikkunassa.")
